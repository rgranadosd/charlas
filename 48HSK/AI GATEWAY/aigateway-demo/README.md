...
Use the `run_demo.sh` script which automatically:
- Verifies that WSO2 API Manager is running and accessible
- Sets up the virtual environment if needed
- Installs dependencies (handles Python 3.14+ compatibility)
...
The script will:
1. **Check APIM Status**: Verify that the WSO2 API Manager is accessible before starting
2. **Setup Environment**: Create and activate virtual environment if needed
3. **Install Dependencies**: Install all required packages
4. **Launch Demo**: Start Streamlit on port 8502

If the APIM is not accessible, you'll be warned and can choose to continue anyway or fix the issue first.
...
Sensitive credentials are stored in a `.env` file that is not tracked by git. The file supports both shared and application-specific credentials:

```env
# WSO2 API Manager Configuration
# Configure the APIM host and port (used by run_demo.sh for connectivity checks)
# WSO2_TOKEN_URL is automatically constructed from WSO2_APIM_HOST and WSO2_APIM_PORT
WSO2_APIM_HOST=localhost
WSO2_APIM_PORT=9453

# WSO2 Gateway Shared Credentials (fallback for all applications)
WSO2_CONSUMER_KEY=your_shared_consumer_key
WSO2_CONSUMER_SECRET=your_shared_consumer_secret

# Provider-specific Chat Completions URLs
# URLs of the WSO2 gateway endpoints for each LLM provider
...
**Configuration Notes:**
- **APIM Configuration**: `WSO2_APIM_HOST` and `WSO2_APIM_PORT` are used by `run_demo.sh` to verify APIM connectivity. The `WSO2_TOKEN_URL` is automatically constructed from these values.
...
3. **Add application-specific credentials** to `.env` (optional, will fallback to shared credentials):
   ...
   # Token URL is automatically constructed from WSO2_APIM_HOST and WSO2_APIM_PORT
   # Only add NEWAPP_TOKEN_URL if you need a different token endpoint for this application
   ```
