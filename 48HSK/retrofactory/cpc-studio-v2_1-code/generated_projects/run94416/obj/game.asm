...
_game_render::
	...
	push	af
	push	af
;src/game.c:286: cpct_drawSolidBox(CPCT_VMEM_START, cpct_px2byteM0(0, 0), 80, 200);
	ld	hl, #0x0000
	push	hl
...
;src/game.c:288: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 60, 18), cpct_px2byteM0(5, 5), 6, 10);
	ld	hl, #0x0505
	push	hl
	call	_cpct_px2byteM0
	ld	d, l
	...
	call	_cpct_drawSolidBox
	pop	af
;src/game.c:289: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 58, 20), cpct_px2byteM0(8, 8), 2, 6);
	inc	sp
	ld	hl,#0x0808
	ex	(sp),hl
	call	_cpct_px2byteM0
	...
	pop	af
	inc	sp
;src/game.c:291: for (i = 0; i < MAX_PROJECTILES; ++i) {
	ld	-4 (ix), #0x00
00119$:
;src/game.c:292: if (g_projectiles[i].active) {
	ld	c,-4 (ix)
	ld	b,#0x00
	ld	l, c
	ld	h, b
	...
	or	a, a
	jr	Z,00102$
;src/game.c:293: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, g_projectiles[i].x, g_projectiles[i].y), cpct_px2byteM0(11, 11), g_projectiles[i].w, g_projectiles[i].h);
	ld	l, c
	ld	h, b
	ld	de, #0x0005
	add	hl, de
	...
	pop	bc
00102$:
;src/game.c:295: projectilerender(&g_projectiles[i]);
	push	bc
...
;src/game.c:291: for (i = 0; i < MAX_PROJECTILES; ++i) {
	...
	sub	a, #0x06
	jr	C,00119$
;src/game.c:298: for (i = 0; i < MAX_ENEMIES; ++i) {
	ld	-4 (ix), #0x00
00121$:
;src/game.c:299: if (g_enemies[i].active) {
	ld	c,-4 (ix)
	ld	b,#0x00
	ld	l, c
	ld	h, b
	...
	or	a, a
	jr	Z,00105$
;src/game.c:300: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, g_enemies[i].x, g_enemies[i].y), cpct_px2byteM0(4, 4), g_enemies[i].w, g_enemies[i].h);
	push	de
	pop	iy
	ld	a, 5 (iy)
	ld	-2 (ix), a
	...
	pop	de
00105$:
;src/game.c:302: enemyrender(&g_enemies[i]);
	push	de
	call	_enemyrender
	pop	af
;src/game.c:298: for (i = 0; i < MAX_ENEMIES; ++i) {
	...
	sub	a, #0x06
	jr	C,00121$
;src/game.c:305: if (g_bossactive) {
	ld	a,(#_g_bossactive + 0)
	or	a, a
	jr	Z,00108$
;src/game.c:306: enemyrender(&g_boss);
	ld	hl, #_g_boss
	push	hl
	call	_enemyrender
;src/game.c:307: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 24, 10), cpct_px2byteM0(1, 1), 32, 2);
	ld	hl, #0x0101
	ex	(sp),hl
	call	_cpct_px2byteM0
	ld	c, l
	...
	pop	af
	inc	sp
;src/game.c:308: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 24, 10), cpct_px2byteM0(5, 5), (u8)(g_boss.health * 3), 2);
	ld	a, (#_g_boss + 7)
	ld	c, a
	add	a, a
	add	a, c
	...
	inc	sp
00108$:
;src/game.c:311: if (!g_pickuptaken) {
	ld	a,(#_g_pickuptaken + 0)
	or	a, a
	jr	NZ,00110$
;src/game.c:312: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 36, (u8)(tilemap_ground_y() - 8)), cpct_px2byteM0(7, 7), 4, 4);
	ld	hl, #0x0707
	push	hl
	call	_cpct_px2byteM0
	ld	c, l
	...
	inc	sp
00110$:
;src/game.c:314: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, g_player.x, g_player.y), cpct_px2byteM0(6, 6), g_player.w, g_player.h);
	ld	hl, #_g_player + 5
	ld	c, (hl)
	ld	hl, #_g_player + 4
	ld	b, (hl)
	...
	call	_cpct_drawSolidBox
	pop	af
;src/game.c:315: playerrender(&g_player);
	inc	sp
	ld	hl,#_g_player
	ex	(sp),hl
	call	_playerrender
	pop	af
;src/game.c:316: hudrender();
	call	_hudrender
;src/game.c:318: if (g_victory) {
	ld	a,(#_g_victory + 0)
	or	a, a
	jr	Z,00117$
;src/game.c:319: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 24, 68), cpct_px2byteM0(8, 8), 32, 12);
	ld	hl, #0x0808
	push	hl
	call	_cpct_px2byteM0
	ld	c, l
	...
	call	_cpct_drawSolidBox
	pop	af
;src/game.c:320: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 28, 72), cpct_px2byteM0(5, 5), 24, 8);
	inc	sp
	ld	hl,#0x0505
	ex	(sp),hl
	call	_cpct_px2byteM0
	...
	jp	00123$
00117$:
;src/game.c:321: } else if (g_gameover) {
	ld	a,(#_g_gameover + 0)
	or	a, a
	jr	Z,00114$
;src/game.c:322: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 24, 68), cpct_px2byteM0(1, 1), 32, 12);
	ld	hl, #0x0101
	push	hl
	call	_cpct_px2byteM0
	ld	d, l
	...
	call	_cpct_drawSolidBox
	pop	af
;src/game.c:323: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 28, 72), cpct_px2byteM0(6, 6), 24, 8);
	inc	sp
	ld	hl,#0x0606
	ex	(sp),hl
	call	_cpct_px2byteM0
	...
	jr	00123$
00114$:
;src/game.c:324: } else if (g_checkpointactive) {
	ld	a,(#_g_checkpointactive + 0)
	or	a, a
	jr	Z,00123$
;src/game.c:325: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, g_checkpointx, (u8)(g_checkpointy - 8)), cpct_px2byteM0(9, 9), 2, 8);
	ld	hl, #0x0909
	push	hl
	call	_cpct_px2byteM0
	ld	c, l
