...
void game_render(void) {
    ...
    cpct_drawSolidBox(CPCT_VMEM_START, cpct_px2byteM0(0, 0), 80, 200);
    tilemap_render();
    cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 60, 18), cpct_px2byteM0(5, 5), 6, 10);
    cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 58, 20), cpct_px2byteM0(8, 8), 2, 6);

    for (i = 0; i < MAX_PROJECTILES; ++i) {
        if (g_projectiles[i].active) {
            cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, g_projectiles[i].x, g_projectiles[i].y), cpct_px2byteM0(11, 11), g_projectiles[i].w, g_projectiles[i].h);
        }
    ...
    for (i = 0; i < MAX_ENEMIES; ++i) {
        if (g_enemies[i].active) {
            cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, g_enemies[i].x, g_enemies[i].y), cpct_px2byteM0(4, 4), g_enemies[i].w, g_enemies[i].h);
        }
    ...
    cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, g_player.x, g_player.y), cpct_px2byteM0(6, 6), g_player.w, g_player.h);
