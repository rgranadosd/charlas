...
; ---------------------------------
_hudrender::
;src/systems/hud.c:67: for (i = 0; i < currenthealth; ++i) {
	ld	c, #0x00
00103$:
	ld	hl, #_currenthealth
;src/systems/hud.c:68: pvmem = cpct_getScreenPtr(CPCT_VMEM_START, (i * 8), 2);
	ld	a,c
	cp	a,(hl)
	jr	NC,00101$
	rlca
...
;src/systems/hud.c:67: for (i = 0; i < currenthealth; ++i) {
	...
	jr	00103$
00101$:
;src/systems/hud.c:72: scoretemp = currentscore;
	ld	hl, (_currentscore)
;src/systems/hud.c:73: hud_draw_digits(scoretemp, 4, 24, 2);
	ld	bc, #0x0218
	push	bc
	ld	a, #0x04
	push	af
	...
	pop	af
	inc	sp
;src/systems/hud.c:75: timetemp = currenttime;
	ld	hl,#_currenttime + 0
	ld	c, (hl)
;src/systems/hud.c:76: hud_draw_digits((u16)timetemp, 3, 56, 2);
	ld	b, #0x00
	ld	hl, #0x0238
	push	hl
	ld	a, #0x03
	...
	call	_hud_draw_digits
	pop	af
;src/systems/hud.c:78: pvmem = cpct_getScreenPtr(CPCT_VMEM_START, 2, 180);
	inc	sp
	ld	hl,#0xb402
	ex	(sp),hl
	ld	hl, #0xc000
	push	hl
	call	_cpct_getScreenPtr
;src/systems/hud.c:79: cpct_drawSprite((u8*)hudlives, pvmem, 8, 8);
	ld	bc, #_hudlives+0
	ld	de, #0x0808
	push	de
	push	hl
	push	bc
	call	_cpct_drawSprite
;src/systems/hud.c:81: pvmem = cpct_getScreenPtr(CPCT_VMEM_START, 12, 180);
	ld	hl, #0xb40c
	push	hl
	ld	hl, #0xc000
	push	hl
	call	_cpct_getScreenPtr
;src/systems/hud.c:82: cpct_drawSprite((u8*)hud_get_number_sprite(currentlives % 10), pvmem, 8, 8);
	push	hl
	ld	a, #0x0a
	push	af
	inc	sp
	...
	push	hl
	call	_cpct_drawSprite
;src/systems/hud.c:84: pvmem = cpct_getScreenPtr(CPCT_VMEM_START, 70, 180);
	ld	hl, #0xb446
	push	hl
	ld	hl, #0xc000
	push	hl
	call	_cpct_getScreenPtr
;src/systems/hud.c:85: cpct_drawSprite((u8*)hud_get_number_sprite(currentweapon % 10), pvmem, 8, 8);
	push	hl
	ld	a, #0x0a
	push	af
	inc	sp
