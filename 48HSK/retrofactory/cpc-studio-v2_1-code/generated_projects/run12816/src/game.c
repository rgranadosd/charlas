...
static void spawn_wave(u8 wave) {
    ...
    for (i = 0; i < count; ++i) {
        enemyspawn(&g_enemies[i], (u8)(48 + (i * 10)), 112, (u8)((i & 1) ? 1 : 0));
    }
...
static void try_fire_projectile(void) {
    ...
    for (i = 0; i < MAX_PROJECTILES; ++i) {
        if (!g_projectiles[i].active) {
            projectilefire(&g_projectiles[i], (u8)(g_player.x + 2), (u8)(g_player.y + 6), dir, g_weapon);
            g_weapon ^= 1;
            g_shootcooldown = 10;
            break;
        }
...
void game_update(void) {
    ...
    if (g_aliveenemies == 0 && !g_gameover) {
        ...
        } else if (g_player.x >= tilemap_goal_x()) {
            g_victory = 1;
        }
    }
    ...
    hudupdate(g_lives, g_score, g_timeleft, g_weapon);
}
