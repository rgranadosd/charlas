...
00225$:
	...
	ld	(de), a
00119$:
;src/entities/player.c:53: if (player->vx < -KPLAYERMOVESPEED) player->vx = (i8)(-KPLAYERMOVESPEED);
	ld	a, (de)
	xor	a, #0x80
	sub	a, #0x7d
	jr	NC,00121$
...
;src/entities/player.c:98: pvmem = cpct_getScreenPtr(CPCT_VMEM_START, player->x, player->y);
	...
	ld	b, h
	pop	de
;src/entities/player.c:99: cpct_drawSolidBox(pvmem, cpct_px2byteM0(6, 6), player->w, player->h);
	push	de
	pop	iy
	ld	a, 5 (iy)
	ld	-1 (ix), a
