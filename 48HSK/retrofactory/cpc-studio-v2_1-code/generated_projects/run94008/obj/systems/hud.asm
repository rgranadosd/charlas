...
;src/systems/hud.c:46: currentlives  = 3;
	...
	ld	(hl), #0x03
	ret
;src/systems/hud.c:49: void hudupdate(u8 health, u16 score, u8 time, u8 lives) {
;	---------------------------------
; Function hudupdate
; ---------------------------------
_hudupdate::
;src/systems/hud.c:50: currenthealth = health;
	ld	hl, #2+0
	add	hl, sp
	ld	a, (hl)
	ld	(#_currenthealth + 0),a
;src/systems/hud.c:51: currentscore  = score;
	ld	hl, #3+0
	add	hl, sp
	ld	a, (hl)
...
;src/systems/hud.c:52: currenttime   = time;
	ld	hl, #5+0
	add	hl, sp
	ld	a, (hl)
	ld	(#_currenttime + 0),a
;src/systems/hud.c:53: currentlives  = lives;
	ld	hl, #6+0
	add	hl, sp
	ld	a, (hl)
	ld	(#_currentlives + 0),a
	ret
;src/systems/hud.c:56: void hudrender(void) {
;	---------------------------------
; Function hudrender
; ---------------------------------
_hudrender::
;src/systems/hud.c:62: for (i = 0; i < currenthealth; ++i) {
	ld	c, #0x00
00103$:
	ld	hl, #_currenthealth
;src/systems/hud.c:63: pvmem = cpct_getScreenPtr(CPCT_VMEM_START, 2 + (i * 8), 2);
	ld	a,c
	cp	a,(hl)
	jr	NC,00101$
	rlca
...
;src/systems/hud.c:62: for (i = 0; i < currenthealth; ++i) {
	...
	jr	00103$
00101$:
;src/systems/hud.c:67: scoretemp = currentscore;
	ld	hl, (_currentscore)
;src/systems/hud.c:68: hud_draw_digits(scoretemp, 5, 88, 2);
	ld	bc, #0x0258
	push	bc
	ld	a, #0x05
	push	af
	...
	pop	af
	inc	sp
;src/systems/hud.c:70: timetemp = currenttime;
	ld	hl,#_currenttime + 0
	ld	c, (hl)
;src/systems/hud.c:71: hud_draw_digits((u16)timetemp, 3, 56, 2);
	ld	b, #0x00
	ld	hl, #0x0238
	push	hl
	ld	a, #0x03
	...
	call	_hud_draw_digits
	pop	af
;src/systems/hud.c:73: pvmem = cpct_getScreenPtr(CPCT_VMEM_START, 2, 180);
	inc	sp
	ld	hl,#0xb402
	ex	(sp),hl
	ld	hl, #0xc000
	push	hl
	call	_cpct_getScreenPtr
;src/systems/hud.c:74: cpct_drawSprite((u8*)hudlives, pvmem, 8, 8);
	ld	bc, #_hudlives+0
	ld	de, #0x0808
	push	de
	push	hl
	push	bc
	call	_cpct_drawSprite
;src/systems/hud.c:76: pvmem = cpct_getScreenPtr(CPCT_VMEM_START, 12, 180);
	ld	hl, #0xb40c
	push	hl
	ld	hl, #0xc000
	push	hl
	call	_cpct_getScreenPtr
;src/systems/hud.c:77: cpct_drawSprite((u8*)hudnumbers[currentlives % 10], pvmem, 8, 8);
	push	hl
	ld	a, #0x0a
	push	af
	inc	sp
