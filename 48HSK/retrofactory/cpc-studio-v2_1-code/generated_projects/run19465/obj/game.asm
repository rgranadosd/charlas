...
;src/game.c:133: cpct_setVideoMode(1);
	ld	l, #0x01
	call	_cpct_setVideoMode
;src/game.c:134: cpct_setPalette((u8*)gpalette16, 16);
	ld	hl, #0x0010
	push	hl
	ld	hl, #_gpalette16
	push	hl
	call	_cpct_setPalette
;src/game.c:135: cpct_setBorder(gpalette16[0]);
	ld	hl, #_gpalette16 + 0
	ld	b, (hl)
	push	bc
	inc	sp
...
;src/game.c:298: enemyrender(&g_boss);
	...
	push	hl
	call	_enemyrender
;src/game.c:299: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 24, 10), 0x44, 32, 2);
	ld	hl, #0x0a18
	ex	(sp),hl
	ld	hl, #0xc000
	push	hl
	...
	pop	af
	inc	sp
;src/game.c:300: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 24, 10), 0x5C, (u8)(g_boss.health * 3), 2);
	ld	a, (#_g_boss + 7)
	ld	c, a
	add	a, a
	add	a, c
...
;src/game.c:303: if (!g_pickuptaken) {
	...
	or	a, a
	jr	NZ,00106$
;src/game.c:304: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 36, (u8)(tilemap_ground_y() - 8)), 0xEE, 4, 4);
	call	_tilemap_ground_y
	ld	a, l
	add	a, #0xf8
	ld	b, a
...
;src/game.c:309: if (g_victory) {
	...
	or	a, a
	jr	Z,00113$
;src/game.c:310: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 24, 68), 0x5A, 32, 12);
	ld	hl, #0x4418
	push	hl
	ld	hl, #0xc000
	push	hl
	...
	call	_cpct_drawSolidBox
	pop	af
;src/game.c:311: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 28, 72), 0x5C, 24, 8);
	inc	sp
	ld	hl,#0x481c
	ex	(sp),hl
	ld	hl, #0xc000
...
;src/game.c:312: } else if (g_gameover) {
	...
	or	a, a
	jr	Z,00110$
;src/game.c:313: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 24, 68), 0x44, 32, 12);
	ld	hl, #0x4418
	push	hl
	ld	hl, #0xc000
	push	hl
	...
	call	_cpct_drawSolidBox
	pop	af
;src/game.c:314: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, 28, 72), 0x4C, 24, 8);
	inc	sp
	ld	hl,#0x481c
	ex	(sp),hl
	ld	hl, #0xc000
...
;src/game.c:315: } else if (g_checkpointactive) {
	...
	or	a, a
	ret	Z
;src/game.c:316: cpct_drawSolidBox(cpct_getScreenPtr(CPCT_VMEM_START, g_checkpointx, (u8)(g_checkpointy - 8)), 0x3A, 2, 8);
	ld	a,(#_g_checkpointy + 0)
	add	a, #0xf8
	ld	b, a
	push	bc
