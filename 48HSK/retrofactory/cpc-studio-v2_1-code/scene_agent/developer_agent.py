...
def input_from_taskdef(taskdef, project_name: str = "testproject") -> DevelopmentInput:
    ...
    is_audio_task = getattr(taskdef, "subagent", "") == "audio_c_agent"
    return DevelopmentInput(
        ...
        constraints=[
            ...
            *([] if is_audio_task else [
                "Audio API available via #include \"audio.h\" (generated by the audio "
                "agent): call audio_init() once in init_game(), audio_update() at end "
                "of main loop, audio_play_sfx(<SFX_* constants defined in audio.h>) on "
                "game events. Do NOT implement audio — just call the API.",
            ]),
            getattr(taskdef, "implementation_hint", "") or "",
        ],
        target_files=["src/audio.h", "src/audio.c"] if is_audio_task else ["src/main.c"],
    )
