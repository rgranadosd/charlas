...
target_op = next((o for o in ops if (o.get("target") == "/v1/chat/completions" or o.get("target") == "/*") and o.get("verb") == "POST"), None)
if not target_op: target_op = ops[0] if ops else {}
req_policies = target_op.get("operationPolicies", {}).get("request", [])
...
# 3. Get Oauth Token
TOKEN_URL = env_vars.get("WSO2_TOKEN_URL", "https://localhost:9453/oauth2/token")
CLIENT_ID = env_vars.get("WSO2_CONSUMER_KEY")
CLIENT_SECRET = env_vars.get("WSO2_CONSUMER_SECRET")
...
# 4. Define Gateway URL
GATEWAY_URL = env_vars.get("MISTRAL_CHAT_COMPLETIONS_URL", "https://localhost:8253/mistralaiapi/0.0.2/v1/chat/completions")
...
def test_gateway(name, content, expected_status, exp_body=None):
    ...
    if expected_status != 200 and exp_body and exp_body not in body:
        success = False
    print("DONE")
